...
function mt:add_content()
    ...
    elseif  finds(rand_name,'天阶','地阶','玄阶','黄阶') then
        ...
        tran_player:sendMsg('|cffffe799【系统消息】|r |cff00ffff'..player:get_name()..'|r 解开了|cff00ff00'..self.name..'|r的封印，原来它是 |cff'..ac.color_code[color or '白']..'【技能书】'..name..'|r',2)
    ...
    elseif rand_name == '书呆子' then
        ...
        if ac.g_game_degree_attr > p:Map_GetServerValue(key) then 
            ...
            if not skl  then 
                ...
                skl:upgrade(1)  
                ac.player.self:sendMsg('|cffffe799【系统消息】|r|cff00ffff'..player:get_name()..'|r 解开了|cff00ff00'..self.name..'的封印|r |cffff0000【可存档成就】'..rand_name..'得到升级|r 升级后的属性可在最强魔灵-隐藏成就中查看',6)
                ac.player.self:sendMsg('|cffffe799【系统消息】|r|cff00ffff'..player:get_name()..'|r 解开了|cff00ff00'..self.name..'的封印|r |cffff0000【可存档成就】'..rand_name..'得到升级|r 升级后的属性可在最强魔灵-隐藏成就中查看',6)
                ac.player.self:sendMsg('|cffffe799【系统消息】|r|cff00ffff'..player:get_name()..'|r 解开了|cff00ff00'..self.name..'的封印|r |cffff0000【可存档成就】'..rand_name..'得到升级|r 升级后的属性可在最强魔灵-隐藏成就中查看',6)
            end   
        else 
