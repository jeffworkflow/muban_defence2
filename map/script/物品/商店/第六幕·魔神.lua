...
local function check(p)
    ...
    if p.cnt_total_succ == 7 and not ac.flag_tzhl  then
        ac.flag_tzhl = true
        local shop = ac.find_unit('生肖十二魂')
        local it = shop:add_sell_item('挑战武伯都')
    ...
    if p.cnt_total_succ == 11 and not ac.flag_tzll  then
        ac.flag_tzll = true
        local shop = ac.find_unit('生肖十二魂')
        local it = shop:add_sell_item('挑战辰龙')
    ...
    if p.cnt_total_succ == 15 and not ac.flag_tzml  then
        ac.flag_tzll = true
        local shop = ac.find_unit('生肖十二魂')
        local it = shop:add_sell_item('挑战马不凡')
        
        ac.player.self:sendMsg('|cffebb608【系统】|cff00ffff '..p:get_name()..' |cff00ff00开启了“|cffff0000挑战马不凡|cff00ff00”，在基地右边的npc-生肖十二魂中查看 ',5)
        ac.player.self:sendMsg('|cffebb608【系统】|cff00ffff '..p:get_name()..' |cff00ff00开启了“|cffff0000挑战马不凡|cff00ff00”，在基地右边的npc-生肖十二魂中查看 ',5)
        ac.player.self:sendMsg('|cffebb608【系统】|cff00ffff '..p:get_name()..' |cff00ff00开启了“|cffff0000挑战马不凡|cff00ff00”，在基地右边的npc-生肖十二魂中查看 ',5)
    end
